...
Local $DeviceList = GUICtrlCreateListView("#|Serial|Connection", 30, 36, 260, 90, BitOR($GUI_SS_DEFAULT_LISTVIEW,$WS_HSCROLL,$WS_BORDER,$BS_CENTER), BitOR($WS_EX_CLIENTEDGE,$LVS_EX_GRIDLINES,$LVS_EX_CHECKBOXES,$LVS_EX_FULLROWSELECT))
...
Func DeviceRefresh()
	...
	Next
	;$item1 = _GUICtrlListView_GetItemText($DeviceList, 0, 1)
	;ConsoleWrite('(' & @ScriptLineNumber & ')' & $item1 & @CRLF)
	;$item2 = _GUICtrlListView_GetItemText($DeviceList, 1, 1)
	;ConsoleWrite('(' & @ScriptLineNumber & ')' & $item2 & @CRLF)
...
;For $i = 0 To UBound($IP_array)-1
    ;If _IsValidIP($IP_array[$i]) = 1 Then
        ;_GUICtrlListView_GetItemText($DeviceList, 0, 1)
        ;_GUICtrlListView_SetItemText($DeviceList, 0, 1)
